...
Language features some of the brainrot and slang words used as keywords. Have fun!
...
Getting/setting dynamic keys is possible using global `gang` variable:
...
Functions can also be ran after a specified timespan using `lag`:
...
And functions can be ran continously using `yappachino`:
...
If imported, the result will be an object which you can do obj.brosYapping1 and obj.brosYapping2
...
```rs
hit_up("https://example.rizz/", { method: "POST", body: "{\"rizzscript\":\"on_god\"}", content_type: "application/json" })(rizz(data) {
  yap(data)
})
```
...
Use `regex` to match a string with a regex pattern:
